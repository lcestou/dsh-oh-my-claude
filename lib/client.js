...
function renderUsage(block, reply) {
  ...
  for (const w of reply.windows) {
    ...
    row.style.cssText = "display:grid;grid-template-columns:1fr auto;align-items:baseline;column-gap:12px;row-gap:3px;padding:3px 0";
...
function watchTurnStatus(ctx) {
  ...
  const attach = async (el) => {
    ...
    if (!provider || !provider.startsWith("claude-code"))
      return;
