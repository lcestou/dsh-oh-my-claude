...
function Sessions({ ctx, boxes }) {
  ...
  const rows = import_react.useMemo(() => {
    ...
    return out.toSorted((a, b) => (order.get(a.g.key) ?? 0) - (order.get(b.g.key) ?? 0) || b.s.modifiedAt - a.s.modifiedAt);
...
function SettingsEditor({ open, onToggle }) {
  ...
  const load = () => {
    setBusy(true);
    setError("");
    fetch(`${ROUTE}/settings`).then((r) => readJson(r)).then((b) => {
      setFile(b);
      setText(b.text);
      setEditing(false);
      setSaved("");
...
function followDeepLink(ctx) {
  ...
  const tick = async () => {
    ...
    try {
      ...
      console.warn(`[dsh-llm-claude] deep link failed: ${e instanceof Error ? e.message : String(e)}`);
    }
  };
  tick();
}
...
function watchContextMeter() {
  ...
  const attach = (panel) => {
    ...
    panel.prepend(block);
    loadUsage().then((reply) => renderUsage(rows, reply), (e) => renderUsage(rows, { ok: false, error: e.message }));
  };
  const bubble = (tip) => {
    ...
    loadUsage().then((reply) => {
      line.textContent = reply.ok ? `Claude ${reply.windows.map((w) => `${w.label.toLowerCase()} ${Math.round(w.usedPercent)}%`).join(" · ") || "usage: no limits"}` : `Claude usage: ${reply.error}`;
    }, (e) => {
