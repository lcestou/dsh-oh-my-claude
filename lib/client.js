...
function IdleChip({ sessionId }) {
  const [deadline, setDeadline] = import_react.useState(null);
  const visibleRef = import_react.useRef(true);
  import_react.useEffect(() => {
    let alive = true;
...
function RewindButton({ sessionId, ctx }) {
  const cwd = ctx.sessions.list.getSnapshot()?.byId[sessionId]?.cwd;
  const isClaude = activeClaudeSession(ctx) === sessionId;
  const [open, setOpen] = import_react.useState(false);
  const [prompts, setPrompts] = import_react.useState([]);
  ...
  return /* @__PURE__ */ jsx_runtime.jsxs("span", {
    ...
    children: [
      ...
      open && /* @__PURE__ */ jsx_runtime.jsxs("div", {
        role: "dialog",
        "aria-label": "Rewind",
        style: { ...popover, width: 520, maxHeight: 360 },
        children: [
...
function ChangesButton({ sessionId, ctx }) {
  const isClaude = activeClaudeSession(ctx) === sessionId;
  const [open, setOpen] = import_react.useState(false);
  const [reply, setReply] = import_react.useState(null);
  const [shown, setShown] = import_react.useState(null);
  const rootRef = import_react.useRef(null);
  useDismiss(open, () => setOpen(false), rootRef);
  import_react.useEffect(() => {
    if (!open)
  ...
  return /* @__PURE__ */ jsx_runtime.jsxs("span", {
    ...
    children: [
      ...
      open && /* @__PURE__ */ jsx_runtime.jsx("div", {
        role: "dialog",
        "aria-label": "Changes",
        style: { ...popover, width: 560, maxHeight: 400 },
...
function McpButton({ sessionId, ctx }) {
  const isClaude = activeClaudeSession(ctx) === sessionId;
  const [open, setOpen] = import_react.useState(false);
  const [reply, setReply] = import_react.useState(null);
  ...
  const rootRef = import_react.useRef(null);
  useDismiss(open, () => setOpen(false), rootRef);
  const load = () => fetch(`${ROUTE}/mcp?session=${encodeURIComponent(sessionId)}`).then((r) => readJson(r)).then(setReply).catch((e) => setReply({ ok: false, error: e.message }));
  import_react.useEffect(() => {
    if (!open)
      return;
    setReply(null);
  ...
  const reconnect = async (serverName) => {
    ...
    setNote("");
    try {
      const r = await readJson(await fetch(`${ROUTE}/mcp/reconnect`, {
        method: "POST",
        headers: { "content-type": "application/json" },
...
function apply(ctx) {
  ...
  ctx.slots.inject("conversation.session.header.actions", () => {
    ...
    ctx.slots.register({ name: "conversation.session.header.actions", id: "claude-idle-warn", order: 31 }, (props) => props.sessionId ? /* @__PURE__ */ jsx_runtime.jsx(IdleChip, {
      sessionId: props.sessionId
    }) : null);
    return null;
  });
  ctx.slots.inject("conversation.input.left", () => {
    ...
    ctx.slots.register({ name: "conversation.input.left", id: "claude-rewind", order: 52 }, (props) => props.sessionId ? /* @__PURE__ */ jsx_runtime.jsx(RewindButton, {
      sessionId: props.sessionId,
      ctx
    }) : null);
    ctx.slots.register({ name: "conversation.input.left", id: "claude-changes", order: 53 }, (props) => props.sessionId ? /* @__PURE__ */ jsx_runtime.jsx(ChangesButton, {
      sessionId: props.sessionId,
      ctx
    }) : null);
    ctx.slots.register({ name: "conversation.input.left", id: "claude-mcp", order: 54 }, (props) => props.sessionId ? /* @__PURE__ */ jsx_runtime.jsx(McpButton, {
      sessionId: props.sessionId,
      ctx
    }) : null);
    return null;
