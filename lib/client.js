...
var ensureFoldStyle = () => {
  if (document.getElementById("dsh-oh-my-claude-fold"))
    return;
...
var bindFoldClicks = () => {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (head)
      head.setAttribute(HEAD_MARK, head.getAttribute(HEAD_MARK) === "1" ? "open" : "1");
  });
};
function watchToolFolds() {
  ...
  bindFoldClicks();
  const scan = () => {
    for (const block of document.querySelectorAll(`.md-code-block:not([${FOLD_MARK}])`)) {
      ...
      if (isTool && head instanceof HTMLElement)
        head.setAttribute(HEAD_MARK, "1");
    }
    for (const head of document.querySelectorAll(`p[${HEAD_MARK}]`)) {
