...
function AccessShield({ sessionId, ctx }) {
  ...
  import_react2.useEffect(() => {
    ...
    const start = (trigger) => {
      ...
      ours.setAttribute("aria-label", `${trigger.getAttribute("aria-label") ?? "Access mode"} (Claude)`);
      ...
      const observer = new MutationObserver(() => {
        ...
        if (labelChild instanceof HTMLElement) {
          ...
          const secondSpan = ours.querySelector("span:nth-child(2)");
          if (secondSpan)
            secondSpan.textContent = modeLabel(currentMode) ?? currentMode;
        }
      });
      ...
      const onDocDown = (e) => {
        if (menuEl && !menuEl.contains(e.target) && e.target !== ours)
          closeMenu();
      };
      const onDocKey = (e) => {
        if (e.key === "Escape")
      ...
      const openMenu = async () => {
        ...
        try {
          stateSnapshot = await readJson(await fetch(`${ROUTE}/permission-mode?session=${encodeURIComponent(sessionId)}`));
        } catch {
          stateSnapshot = null;
        }
        if (!stateSnapshot)
          ...
          modeRow.addEventListener("click", () => {
            ...
            const doSet = () => {
              ...
              fetch(`${ROUTE}/permission-mode`, {
                ...
                if (reply.error) {
                  ...
                  if (errLine)
                    errLine.textContent = reply.error;
                } else {
                  openMenu();
                }
              }).catch((e) => {
            ...
            if (need !== have) {
              ...
              if (!live) {
                const errLine = menuEl?.querySelector("[data-err]");
                if (errLine)
                  errLine.textContent = "this session is not materialized yet";
                return;
              }
              live.command(`/permission ${need}`).then((reply) => {
                if (!reply || !reply.ok) {
                  const errLine = menuEl?.querySelector("[data-err]");
                  if (errLine)
                    errLine.textContent = reply?.error?.message ?? "command failed";
                } else {
