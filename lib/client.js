...
// src/client/tailFollow.ts
var distanceFromBottom = (g) => Math.max(0, g.scrollHeight - g.scrollTop - g.clientHeight);
var atBottom = (g, px) => distanceFromBottom(g) <= px;
function nextFollow(prev, signal, g, px) {
  switch (signal) {
    case "turn-start":
      return true;
    case "user-up":
...
function watchTailFollow(ctx) {
  const SLACK_PX = 120;
  const ZERO = { scrollTop: 0, scrollHeight: 0, clientHeight: 0 };
  let follow = true;
  ...
  const scan = () => {
    ...
    if (running && !prevRunning)
      follow = nextFollow(follow, "turn-start", ZERO, SLACK_PX);
    prevRunning = running;
    if (!running)
      return;
