...
function OhMyClaudeControl({ sessionId, ctx }) {
  ...
  const [tab, setTab] = import_react.useState(lastTab);
  const [memoryCount, setMemoryCount] = import_react.useState(null);
  const onCount = import_react.useCallback((n) => setMemoryCount(n), []);
  const [above, setAbove] = import_react.useState(0);
...
function apply(ctx) {
  ...
  ctx.slots.inject("conversation.input.left", () => {
    ctx.slots.register({ name: "conversation.input.left", id: "oh-my-claude", order: 50 }, (props) => props.sessionId ? /* @__PURE__ */ jsx_runtime.jsx(OhMyClaudeControl, {
      sessionId: props.sessionId,
      ctx
    }) : null);
    return null;
