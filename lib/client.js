...
var flushAddedNodes = () => {
  ...
  if (!nodes)
    return;
  for (const handler of addedNodeHandlers)
    for (const node of nodes)
      handler(node);
};
var onAddedNodes = (handler) => {
    ...
    for (const r of records)
      for (const n of r.addedNodes)
        if (n instanceof HTMLElement)
          (pendingNodes ??= []).push(n);
    if (pendingNodes && scanFrame === 0)
      scanFrame = requestAnimationFrame(flushAddedNodes);
  });
