...
import { contextDrops } from "./context-sources.js";
import { accountIdentity, forgetIdentity, readHints, readPickerSettings, readRemoteWorkspaces, readSshBoxes, registerSessionRoutes, sshBoxProviderId, } from "./sessions.js";
...
import { authHeaders, auxCwd, buildRedactor, CLAUDE_HOME, hasPendingNotice, loadAsides, saveAsides, loadStarters, saveStarter, loadStarted, isPermissionMode, loadPermissionModes, loadToolMode, saveToolMode, lastSelectedProvider, loadLimitWaits, loadTurnRecords, saveLimitWait, markBusy, modesUpTo, noteBoot, PERMISSION_MODES, loadCommandCatalog, rememberStarted, resolveClaudeHome, saveCommandCatalog, savePermissionMode, saveTurnRecords, STATE_DIR, stateDir, takeInterrupted, trace, loadWatches, saveWatch, loadTerminalSync, saveTerminalSync, saveWorkspaceModel, } from "./state.js";
import { suggestRule } from "./permissions.js";
import { probeRawToolRows, } from "./rows-probe.js";
...
import { homeAt, readFromAt, sizeAt } from "./remote-fs.js";
...
export function contextSourceOf(m) {
    if (m.source?.kind === "agent-instructions")
        return "instructions";
    if (m.source?.kind === "skill-catalog")
        return "skills";
...
export function attachmentNotes(turns, images) {
    ...
    for (const m of turns) {
        ...
        for (const b of m.content) {
            ...
            notes.push(`[Image ${ref.name ? `"${ref.name}" ` : ""}(${ref.attachmentId}): the copy shown above is saved at "${path}" (${ref.mediaType}${size}). Read that path with your file tools when it is needed again, and copy it to a writable location before modifying it.]`);
        }
    }
    return notes.join("\n");
}
...
export function stepContextFor(messages, drops = new Set()) {
    const parts = [];
...
export class ClaudeCodeAdapter extends LlmAdapter {
    ...
    async listModels(provider) {
        ...
        if (this.loggedOut)
            throw new Error(`not logged in on ${this.config.sshHost || hostname()}. Log in under Settings, Oh My Claude, Boxes`);
    ...
    askSideQuestion(sessionId, question, context) {
    ...
    spawner() {
        ...
        const local = (command, args, cwd) => base(command, args, cwd, this.localEnvOverride());
...
export function apply(ctx, config) {
    ...
    if (adapter.providerId !== "claude-code") {
        ...
        registerUsageRoute(ctx, (level, msg) => adapter.log(level, msg), (home, sshHost) => accountIdentity(adapter.config.command, home, sshHost ?? adapter.config.sshHost), { home: claudeHome, boxFor: usageBoxFor });
