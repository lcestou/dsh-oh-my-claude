...
import { ClaudeProcess, allowResult, answersFor, controlErrorLine, controlResponseLine, denyResult, parseQuestions, permissionReason, userTurnLine, interruptLine, nodeSpawner, seamSpawner, } from "./process.js";
import { ADAPTER_CURRENT, RESUME_TIMER, PROCESS_REGISTRY, asSessionId } from "./dsh.js";
import { CLAUDE_HOME, STATE_DIR, auxCwd, authHeaders, loadStarted, markBusy, rememberStarted, takeInterrupted, trace, } from "./state.js";
    ...
    if (headers) {
        try {
            ...
            if (res.ok) {
                ...
                if (data.length > 0)
                    catalog = { at: Date.now(), models: data.map(modelFromApi) };
                return catalog.models;
            }
        }
        catch {
...
export function buildArgs({ model, reasoningEffort, system, purpose, config, session, accessMode, flags, promptText, mcp, }) {
    const args = ["-p"];
    if (usesStdin(flags))
...
export const RESTART_TEXT = "dsh restarted while this turn was in progress and the Claude Code process was replaced. Pick up where the transcript stops and finish the task.";
...
export class Translator {
    ...
    constructor({ toolActivity = true, toolTextLimit = TOOL_TEXT_LIMIT, relay = false, dshIds, relayed, log, onTodoWrite, } = {}) {
        this.log = log ?? (() => { });
        ...
        for (const b of content) {
            if (b.type === "text" && b.text)
                ...
                if (toolName === "TodoWrite")
                    continue; // mirrored to the todo panel, not shown as a row
                const dsh = toolName.startsWith("mcp__dsh__");
                if (dsh) {
...
export class ClaudeCodeAdapter extends LlmAdapter {
    ...
    async forkTranscript(options, cwd, id) {
        ...
        let text;
        try {
            text = await readFile(join(CLAUDE_HOME, "projects", projectDirName(parentCwd), `${parentClaude}.jsonl`), "utf8");
        }
        catch {
            return false;
        }
    ...
    async resumeInterrupted(path) {
        ...
        await trace(`boot: interrupted=${JSON.stringify(ids)} live=${JSON.stringify([...this.processes.keys()])}`);
        for (const id of ids) {
    ...
    spawner() {
        if (this.config.spawn === "dsh" && this.subprocess)
            return seamSpawner(this.subprocess);
    ...
    async acquire(options, forceFresh) {
        ...
        if (!proc) {
            ...
            proc = new ClaudeProcess({
                ...
                onExit: (p) => {
                    if (this.processes.get(options.sessionId) === p)
    ...
    async *turn(options, forceFresh) {
        ...
        const dispatch = async function* (event) {
            ...
            if (event.type === "control_response" || event.type === "timeout")
                return "continue";
            ...
            yield* tr.translate(event);
            if (tr.toolPending)
                clearTimeout(timer); // tool running: silence is expected, do not time out
            if (tr.finished)
                return "finished";
        ...
        try {
            ...
            if (outcome === "relayed")
                ...
                yield { type: "finish", reason: this.endReason(proc, options, idle) };
        }
        finally {
            clearTimeout(timer);
            ...
            proc.busy = false;
            proc.lastUsed = Date.now();
            markBusy(options.sessionId, false).catch((e) => this.log("warn", `busy.json: ${errorText(e)}`));
    ...
    mirrorTodos(sessionId, todos) {
        if (!this.config.mirrorTodos)
            return;
        try {
    ...
    async wake(sessionId, proc, text = WAKE_TEXT) {
        ...
        if (!agent) {
            ...
            if (text === RESTART_TEXT)
                await trace(`wake ${sessionId}: no agent (${how})`);
            return;
        }
        if (text === RESTART_TEXT)
    ...
    async *handleControl(event, options, prep, proc, pending, tr) {
        ...
        if (request.subtype !== "can_use_tool") {
            proc.write(controlErrorLine(requestId, `${request.subtype ?? "unknown"} is not supported by dsh-llm-claude`));
            return;
        }
...
export function apply(ctx, config) {
    ...
    registerUsageRoute(ctx, (level, msg) => adapter.log(level, msg), () => accountIdentity(adapter.config.command));
