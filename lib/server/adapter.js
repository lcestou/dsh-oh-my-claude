...
export class ClaudeCodeAdapter extends LlmAdapter {
     ...
     */
    async editSteer(sessionId, id, text) {
        const proc = this.processes.get(registryKey(this.providerId, sessionId));
        if (!proc?.alive)
        ...
        if (!reply.ok)
            return { ok: false, reason: "error", error: reply.error };
        ...
        if (!proc.busy) {
            ...
            if (original)
                inbox?.replace(id, { ...original, content: [{ type: "text", text }] });
            return { ok: true };
        }
        const uuid = randomUUID();
        if (!proc.write(buildInput(text, [], uuid)))
        ...
        if (stillWaiting)
            proc.steers.set(id, { uuid, key: waiting.key, text, at: waiting.at });
        if (original)
            inbox?.replace(id, { ...original, content: [{ type: "text", text }] });
        return { ok: true };
    }
...
export function apply(ctx, config) {
    ...
    if (adapter.providerId !== "claude-code") {
        ...
        registerSessionRoutes(ctx, {
            ...
            steersFor: (sessionId) => adapter.ownerFor(sessionId).steersFor(sessionId),
            editSteer: (sessionId, id, text) => adapter.ownerFor(sessionId).editSteer(sessionId, id, text),
