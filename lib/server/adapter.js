...
import { ADAPTER_CURRENT, COMMAND_CATALOG, RESUME_TIMER, PROCESS_REGISTRY, TURN_RECORDS, asSessionId, } from "./dsh.js";
    ...
    bridgeCommands(names, agent) {
        ...
        for (const cmd of names) {
            if (this.bridged.has(cmd))
                continue;
            if (agent && commands.find(agent, cmd) !== undefined)
                continue; // dsh's own wins
            try {
