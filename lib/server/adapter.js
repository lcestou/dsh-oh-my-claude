...
import { ClaudeProcess, allowResult, answersFor, controlErrorLine, controlResponseLine, denyResult, parseQuestions, permissionReason, userTurnLine, interruptLine, nodeSpawner, seamSpawner, attachKeeper, launchKeeper, lazyHandle, pidAlive, readKeeperInfo, readKeeperSpec, spawnKeeper, } from "./process.js";
...
import { authHeaders, auxCwd, buildRedactor, CLAUDE_HOME, hasPendingNotice, loadStarted, markBusy, noteBoot, rememberStarted, resolveClaudeHome, STATE_DIR, stateDir, takeInterrupted, trace, } from "./state.js";
...
export function buildArgs({ model, reasoningEffort, system, purpose, config, session, accessMode, flags, promptText, mcp, temporary = false, }) {
    const args = ["-p"];
    if (usesStdin(flags))
...
export const RECONNECT_TEXT = "dsh restarted and reattached to your still-running Claude Code process; what you did meanwhile is shown above. Continue where you are.";
export const RESTART_TEXT = "dsh restarted while this turn was in progress and the Claude Code process was replaced. Pick up where the transcript stops and finish the task. If this session has an active goal, dsh disarmed it on resume: call get_goal, then update_goal with action resume, so the goal rounds keep driving the work without anyone typing.";
...
export class Translator {
    ...
    callInputs = new Map();
    constructor({ toolActivity = true, toolTextLimit = TOOL_TEXT_LIMIT, relay = false, dshIds, relayed, log, onToolCall, onToolResult, onResult, redact, onInit, } = {}) {
        this.log = log ?? (() => { });
...
export class ClaudeCodeAdapter extends LlmAdapter {
    ...
    keeperEnv() {
        ...
        if (this.config.configDir)
            env.CLAUDE_CONFIG_DIR = this.claudeHome;
        return env;
    }
    ...
    spawner() {
        ...
        return (command, args, cwd) => base(command, args, cwd, envOverride);
    }
    ...
    async *turn(options, forceFresh) {
        ...
        const tr = new Translator({
            ...
            onToolCall: turnStep && this.config.toolActivity
            ...
            onToolResult: turnStep && this.config.toolActivity
...
export function apply(ctx, config) {
    ...
    if (adapter.providerId !== "claude-code") {
        ...
        registerUsageRoute(ctx, (level, msg) => adapter.log(level, msg), () => accountIdentity(adapter.config.command), claudeHome);
        registerSessionRoutes(ctx, {
            log: (level, msg) => adapter.log(level, msg),
            projectDir: (cwd) => join(claudeHome, "projects", projectDirName(cwd)),
