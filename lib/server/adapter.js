...
import { accountIdentity, readPickerSettings, readSshBoxes, registerSessionRoutes, sshBoxProviderId, } from "./sessions.js";
...
import { authHeaders, auxCwd, buildRedactor, CLAUDE_HOME, hasPendingNotice, loadStarted, isPermissionMode, loadPermissionModes, lastSelectedProvider, loadLimitWaits, loadTurnRecords, saveLimitWait, markBusy, modesUpTo, noteBoot, PERMISSION_MODES, rememberStarted, resolveClaudeHome, savePermissionMode, saveTurnRecords, STATE_DIR, stateDir, takeInterrupted, trace, } from "./state.js";
...
export class ClaudeCodeAdapter extends LlmAdapter {
    ...
    keeperEnv() {
        return childEnv(process.env, this.config.configDir ? { CLAUDE_CONFIG_DIR: this.claudeHome } : undefined);
    }
    /** Spawner for one session: keeper mode needs the session to place and name the keeper. */
    spawnerFor(sessionId, spec) {
        if (this.config.spawn !== "keeper" || !sessionId || this.config.sshHost)
            return this.spawner();
    ...
    spawner() {
        ...
        if (this.config.sshHost)
            return sshSpawner(this.config.sshHost);
