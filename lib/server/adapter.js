...
import { authHeaders, auxCwd, buildRedactor, CLAUDE_HOME, hasPendingNotice, loadStarted, isPermissionMode, loadPermissionModes, loadTurnRecords, markBusy, noteBoot, rememberStarted, resolveClaudeHome, savePermissionMode, saveTurnRecords, STATE_DIR, stateDir, takeInterrupted, trace, } from "./state.js";
...
export async function getCatalog(fetchImpl = fetch, cli = []) {
...
export function apply(ctx, config) {
    ...
    if (adapter.providerId !== "claude-code") {
        ...
        registerUsageRoute(ctx, (level, msg) => adapter.log(level, msg), (home) => accountIdentity(adapter.config.command, home), { home: claudeHome, homeFor });
