...
import { authHeaders, auxCwd, buildRedactor, CLAUDE_HOME, hasPendingNotice, loadStarted, isPermissionMode, loadPermissionModes, markBusy, noteBoot, rememberStarted, resolveClaudeHome, savePermissionMode, STATE_DIR, stateDir, takeInterrupted, trace, } from "./state.js";
...
export function mergeCatalog(cli, base) {
    if (cli.length === 0)
        return base;
...
export function setCliModels(models) {
    cliModels = models;
}
...
export class ClaudeCodeAdapter extends LlmAdapter {
    ...
    async adoptKeepers() {
        ...
        for (const dir of dirs) {
            ...
            if (!info ||
                ...
                await trace(join(this.stateDir, "resume.log"), `dropping keeper ${basename(dir)} for ${info?.sessionId ?? "?"}: exit=${JSON.stringify(info?.exit ?? null)} endedBy=${info?.endedBy ?? "?"} keeperAlive=${info ? pidAlive(info.pid) : "?"} claudeAlive=${info ? pidAlive(info.claudePid) : "?"} spec=${spec ? "ok" : "missing"}`);
    ...
    async acquire(options, forceFresh) {
        ...
        if (proc?.alive && !proc.busy && proc.key !== key)
    ...
    async *turn(options, forceFresh) {
        ...
        const onAbort = () => {
            ...
            if (!interruptOnAbort(abortKind(options.signal), this.config.spawn))
                return;
        ...
        const dispatch = async function* (event) {
            ...
            yield* tr.translate(event);
            if (tr.toolPending)
                clearIdle(options.sessionId); // tool running: silence is expected
            if (tr.finished)
                return "finished";
    ...
    async *elicit(request, requestId, options, proc, pending, tr) {
        ...
        if (request.mode === "url") {
            yield* tr.wholeBlock("reasoning", `❓ ${who} wants a browser step: ${request.url ?? "(no url)"} — declined`);
