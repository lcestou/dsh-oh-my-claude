...
import { ADAPTER_CURRENT, COMMAND_CATALOG, PERMISSION_MODE_OVERRIDES, RESUME_TIMER, PROCESS_REGISTRY, TEMPORARY_SESSIONS, TURN_RECORDS, asSessionId, } from "./dsh.js";
import { authHeaders, auxCwd, buildRedactor, CLAUDE_HOME, hasPendingNotice, loadAsides, saveAsides, loadStarters, saveStarter, loadStarted, isPermissionMode, loadPermissionModes, loadToolMode, saveToolMode, lastSelectedProvider, loadLimitWaits, loadTurnRecords, saveLimitWait, markBusy, modesUpTo, noteBoot, PERMISSION_MODES, loadCommandCatalog, rememberStarted, resolveClaudeHome, saveCommandCatalog, savePermissionMode, saveTurnRecords, STATE_DIR, stateDir, takeInterrupted, trace, } from "./state.js";
...
export class ClaudeCodeAdapter extends LlmAdapter {
    ...
    transcriptPath(prep) {
        ...
        if (!id || this.config.sshHost || remoteWorkspaceFor(prep.cwd))
            return undefined;
    ...
    async terminalTurnsSince(proc, prep) {
        const path = this.transcriptPath(prep);
        if (!path || proc.transcriptSeen === undefined)
            return [];
        try {
            return foreignTurns(await readTranscriptFrom(path, proc.transcriptSeen), CHILD_ENV.CLAUDE_CODE_ENTRYPOINT);
        }
        catch (error) {
    ...
    async markTranscriptSeen(proc, prep) {
        const path = this.transcriptPath(prep);
        if (!path)
            return;
        try {
