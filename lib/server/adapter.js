...
import { ClaudeProcess, allowResult, answersFor, controlErrorLine, controlResponseLine, denyResult, parseQuestions, permissionReason, userTurnLine, interruptLine, setPermissionModeLine, nodeSpawner, seamSpawner, attachKeeper, launchKeeper, lazyHandle, pidAlive, readKeeperInfo, readKeeperSpec, spawnKeeper, } from "./process.js";
...
export class ClaudeCodeAdapter extends LlmAdapter {
    ...
    async setPermissionMode(sessionId, mode) {
        ...
        if (!proc.write(setPermissionModeLine(requestId, info.mode)))
            return { ...info, live: false };
        if (!proc.busy)
            return { ...info, live: true }; // answered when the next turn starts
