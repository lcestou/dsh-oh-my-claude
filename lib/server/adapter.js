...
import { authHeaders, auxCwd, buildRedactor, CLAUDE_HOME, hasPendingNotice, loadStarted, isPermissionMode, loadPermissionModes, loadLimitWaits, loadTurnRecords, saveLimitWait, markBusy, modesUpTo, noteBoot, PERMISSION_MODES, rememberStarted, resolveClaudeHome, savePermissionMode, saveTurnRecords, STATE_DIR, stateDir, takeInterrupted, trace, } from "./state.js";
    ...
    armLimitWait(sessionId, resetAt, atLeastMs = 0) {
        ...
        const timer = setTimeout(() => {
            ...
            this.wake(sessionId, proc, LIMIT_TEXT)
                .then((sent) => trace(log, `limit wait ${sessionId}: fired, notice ${sent ? "sent" : "not sent"}`))
