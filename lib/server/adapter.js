...
export const attachmentsOf = (content) => Array.isArray(content)
    ? content.flatMap((b) => {
        if ((b.type !== "file" && b.type !== "image") || !b.attachment)
            return [];
        // SAFETY: both attachment refs carry `bytes`; only the file one always carries `name`
        const ref = b.attachment;
        const chip = { kind: b.type, bytes: ref.bytes ?? 0 };
        if (ref.name)
            chip.name = ref.name;
        return [chip];
    })
...
export class ClaudeCodeAdapter extends LlmAdapter {
            ...
            if ((event.data?.removedCount ?? 0) > 0) {
                ...
                if (inbox)
                    for (const [id, st] of proc.steers) {
                        if (st.uuid !== undefined || inbox.nextStep.some((m) => m.id === id))
                            continue;
    ...
    async releaseHold(sessionId, holdId, how) {
        ...
        const kept = hold.messages.flatMap((m) => Array.isArray(m.content)
            ...
            : [
                {
                    ...
                    content: [
                        ...kept,
                        ...(how.text.trim() === "" ? [] : [{ type: "text", text: how.text }]),
                    ],
                },
            ];
        for (const m of out)
    ...
    askSideQuestion(sessionId, question, context, quote) {
        ...
        const send = (proc) => owner.control(proc, { subtype: "side_question", question: asked, history: [] }, ASIDE_TIMEOUT_MS);
        ...
        void (live?.alive
            ...
            : owner
                .revive(sessionId)
                .then((proc) => proc?.alive ? send(proc) : { ok: false, error: NO_PROCESS_FOR_ASIDE })).then((reply) => {
            entry.pending = false;
            if (!reply.ok) {
    ...
    async acquire(options, forceFresh) {
        ...
        if (this.lastTurnOptions.size > REVIVE_KEEP) {
            ...
            if (oldest !== undefined)
                this.lastTurnOptions.delete(oldest);
        }
    ...
    async writeSteerMessage(m, key, proc, options, uuid) {
        ...
        if (!proc.write(buildInput(body, images, uuid)))
            return false;
        proc.sent.add(key);
        return true;
    }
    ...
    withFileHandles(m) {
        ...
        if (m.content.some((b) => b.type === "image") && !this.ctx.attachments)
            return undefined;
        const content = [];
        for (const b of m.content) {
            if (b.type !== "file")
