...
export const validSkillName = (name) => /^[a-z0-9][a-z0-9-]{0,63}$/.test(name);
/** The two scopes a person can write. A plugin's skills are edited where the plugin ships them. */
export const isWritableSkillScope = (scope) => scope === "user" || scope === "project";
...
export function skillTargetPath(scope, name, claudeHome, cwd) {
    if (!validSkillName(name))
        return null;
    if (scope === "user")
        return join(claudeHome, "skills", name, "SKILL.md");
    if (scope === "project")
        return cwd ? join(cwd, ".claude", "skills", name, "SKILL.md") : null;
    return null;
}
