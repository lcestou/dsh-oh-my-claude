 ...
 */
export const copyScript = (name) => `d=${FAR_ATTACHMENTS} && mkdir -p "$d" && p="$d"/${shq(name)} && ` +
    `if [ -e "$p" ]; then cat >/dev/null; else cat > "$p.tmp.$$" && mv -- "$p.tmp.$$" "$p"; fi && ` +
    `printf %s "$p"`;
/**
...
export async function copyToAt(box, localPath, name, timeoutMs = 300_000, run = (host, script) => nodeSpawner("ssh", sshArgs(host, script), ".")) {
    ...
    const child = run(host, `printf ${shq(MARK)}; ${copyScript(name)}`);
    let out = "";
    let err = "";
