 ...
 */
export const writeScript = (path, base64) => `mkdir -p ${shq(dirname(path))} && { [ -e ${shq(path)} ] && cp -- ${shq(path)} ${shq(`${path}.bak`)} || true; } && ` +
...
const ssh = (host, script, timeout = 15_000) => new Promise((resolve) => execFile("ssh", sshArgs(host, script), { timeout, windowsHide: true, maxBuffer: 8 * 1024 * 1024 }, (e, out, err) => resolve({
...
const transportError = (host, r) => new Error(`${host}: ${r.err || `ssh exited ${r.code}`}`);
