...
export function registerSessionRoutes(ctx, { log, projectDir, projectsDir, startedIds, claudeIdOf, settingsPath, configDir, boxesPath, command, turnRecords, idle, permissionModes, thinking, rewind, contextUsage, workspaceDiff, mcp, permissionAsks, sideQuestions, models, reloadPlugins, continueAfterLimit, }) {
                ...
                try {
                    ...
                    if (req.method === "POST" &&
                        ...
                        if (ring) {
                            ...
                            if (kept.length > 0)
                                sideQuestions?.set(sid, kept);
                            else
                                sideQuestions?.delete(sid);
                        }
                        return json(res, 200, { ok: true });
                    }
