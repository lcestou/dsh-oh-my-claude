...
const run = (cmd, args, env, cwd) => new Promise((resolve) => execFile(cmd, args, { timeout: 8000, windowsHide: true, env, cwd }, (e, out, err) => resolve(e
    ? {
        out: "",
...
export async function accountIdentity(command = "claude", configDir) {
    const key = configDir ?? "";
 ...
 */
async function settingsTexts(userPath, cwd) {
    const texts = [];
...
async function writeWithBackup(path, text) {
...
export function registerSessionRoutes(ctx, { log, projectDir, projectsDir, startedIds, claudeIdOf, settingsPath, configDir, boxesPath, command, turnRecords, idle, permissionModes, rewind, contextUsage, workspaceDiff, mcp, permissionAsks, models, continueAfterLimit, }) {
            ...
            handler: async (req, res) => {
                ...
                try {
                    ...
                    if (req.method === "POST" && url.pathname === `${ROUTE_PREFIX}/diagnostics/doctor`)
                        return json(res, 200, await run(command || "claude", ["doctor"], {
                            ...process.env,
