...
export async function probeBox(box, fetchImpl = fetch, path = "status") {
    ...
    const status = (cookie) => fetchImpl(`${url}/dsh-oh-my-claude/${path}`, {
...
export function registerSessionRoutes(ctx, { log, projectDir, projectsDir, startedIds, claudeIdOf, settingsPath, configDir, boxesPath, command, }) {
