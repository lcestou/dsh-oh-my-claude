...
export function hasPendingNotice(events, plugin, texts = []) {
    ...
    for (const e of events) {
        ...
        for (const m of d.inserted) {
            ...
            // v3 logs wrap the plugin name in `{ kind: "plugin", plugin }`; v4 logs carry it in the kind.
            if (typeof src === "object" && src !== null) {
                if ("plugin" in src && src.plugin === plugin)
                    pending = true;
                if ("kind" in src && src.kind === `plugin:${plugin}`)
                    pending = true;
            }
...
export const SESSION_REPAIRS_FILE = (d) => join(d, "session-repairs.json");
...
export async function loadSessionRepairs(dir) {
    const empty = { version: 1, lastSweepAt: 0, logs: {} };
    try {
        const parsed = JSON.parse(await readFile(SESSION_REPAIRS_FILE(dir), "utf8"));
        if (typeof parsed !== "object" || parsed === null)
            return empty;
        // SAFETY: a non-null object; each field is checked for its type before use.
        const r = parsed;
        ...
        if (typeof r.logs === "object" && r.logs !== null) {
            for (const [id, raw] of Object.entries(r.logs)) {
                ...
                const e = raw;
                if (typeof e.path !== "string" || typeof e.verdict !== "string")
                    continue;
                if (typeof e.at !== "number")
                    continue;
                if (!["fine", "healed", "unknown", "rolled-back", "owned"].includes(e.verdict))
                    continue;
                const rec = {
                    path: e.path,
...
export function saveSessionRepairs(dir, next) {
 ...
 *  other's verdict. */
export function recordRepair(dir, id, record) {
...
export function saveWorkspaceModel(dir, cwd, model, at = Date.now(), provider) {
