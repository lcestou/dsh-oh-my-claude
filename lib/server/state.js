...
export async function trace(line) {
    try {
...
export async function loadStarted() {
    if (started)
        return started;
    try {
        const ids = JSON.parse(await readFile(STATE_FILE, "utf8"));
        started = new Set(Array.isArray(ids) ? ids.filter((x) => typeof x === "string") : []);
    }
    catch {
        started = new Set();
    }
 ...
 */
export async function rememberStarted(id, keep = true) {
