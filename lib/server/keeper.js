...
function main(dir) {
    ...
    // Linux caps a unix socket path at 108 bytes (sun_path). A state dir deep enough to pass that
    // does not fail as a length error: listen answers EADDRINUSE, and dsh reads the silence as
    // "keeper did not answer". Refuse before Claude is spawned, with the cause where it is looked
    // for (2026-09-18: a scratch DSH_OMC_STATE_DIR under a session scratchpad, 159 bytes).
    const sockBytes = Buffer.byteLength(sockPath);
    if (sockBytes > 100) {
        log(`socket path is ${sockBytes} bytes, over the 108-byte unix socket limit: ${sockPath}`);
    ...
    child.on("error", (e) => {
        if (exit)
            return; // `close` can follow `error`; the first one to land owns the record
    ...
    writeInfo();
    createInterface({ input: child.stdout, crlfDelay: Infinity }).on("line", (line) => send({ t: "out", line }));
    createInterface({ input: child.stderr, crlfDelay: Infinity }).on("line", (line) => send({ t: "err", line }));
    ...
    child.on("close", (code, signal) => {
        if (exit)
            return; // a spawn error already reported this child; do not report it twice
        exit = { code, signal };
        endedBy ??= "child";
    ...
    const server = createServer((sock) => {
        ...
        createInterface({ input: sock, crlfDelay: Infinity }).on("line", (raw) => {
            ...
            if (msg.t === "hello") {
                sock.write(`${JSON.stringify({ t: "hello", pid: process.pid, claudePid: child.pid, buffered: buffer.length, dropped })}\n`);
                if (dropped > 0)
                    sock.write(`${JSON.stringify({ t: "err", line: `keeper: ${dropped} output lines dropped while detached` })}\n`);
                for (const l of buffer)
                    sock.write(l);
                buffer.length = 0;
                dropped = 0;
                if (exit)
                    sock.write(`${JSON.stringify({ t: "exit", ...exit })}\n`);
            }
