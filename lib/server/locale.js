...
const EN = {
    ...
    noTranscriptToReseed: "No Claude Code transcript with a completed turn was found for this session, so there is nothing to reseed from.",
    ...
    listSep: ", ",
    deniedCallsOne: "Claude Code denied {n} tool call that needed approval. Switch Access mode to Full Access to allow them.",
    deniedCallsOther: "Claude Code denied {n} tool calls that needed approval. Switch Access mode to Full Access to allow them.",
    autoBlockedOne: "Auto mode blocked {n} tool call ({reasons}). A Bash permission rule in Claude Code's settings allows such a call; permissionMode bypassPermissions skips the classifier.",
    autoBlockedOther: "Auto mode blocked {n} tool calls ({reasons}). A Bash permission rule in Claude Code's settings allows such a call; permissionMode bypassPermissions skips the classifier.",
    ...
    seededSystemPrompt: "No system prompt recorded: these turns were restored from a Claude Code transcript, and Claude Code ran them with its own prompt.",
    noReply: "(no reply)",
...
const ZH = {
    ...
    deniedCallsOther: "Claude Code 拒绝了 {n} 次需要批准的工具调用。将访问模式切换为完全权限即可允许。",
    autoBlockedOne: "自动模式拦截了 {n} 次工具调用（{reasons}）。在 Claude Code 设置中添加 Bash 权限规则可允许这类调用；permissionMode 设为 bypassPermissions 会跳过分类器。",
    autoBlockedOther: "自动模式拦截了 {n} 次工具调用（{reasons}）。在 Claude Code 设置中添加 Bash 权限规则可允许这类调用；permissionMode 设为 bypassPermissions 会跳过分类器。",
    ...
    seededSystemPrompt: "未记录系统提示：这些轮次从 Claude Code 会话记录恢复，由 Claude Code 自带的提示驱动。",
    noReply: "（无回复）",
