...
export async function readUsage(fetchImpl = fetch) {
...
export function registerUsageRoute(ctx, log, identity) {
    let cached;
    let inFlight;
    // A 429 backs the endpoint off until here, even when the client keeps forcing; serve cache meanwhile.
    let rateLimitedUntil = 0;
    const rateLimited = () => cached?.reply.ok ? cached.reply : { ok: false, error: "usage endpoint rate limited" };
    const read = (force) => {
        if (Date.now() < rateLimitedUntil)
            return Promise.resolve(rateLimited());
        const age = cached ? Date.now() - cached.at : Infinity;
        if (cached && age < (force ? FORCE_MIN_AGE_MS : CACHE_MS))
        ...
        inFlight ??= readUsage().then((reply) => {
            ...
            if (reply.ok || !cached?.reply.ok || age >= CACHE_MS)
                cached = { at: Date.now(), reply };
            inFlight = undefined;
            return cached.reply;
        });
        return inFlight;
