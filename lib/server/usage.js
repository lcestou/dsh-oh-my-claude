...
export function usageWindows(payload) {
        ...
        if (entry.kind === "session")
            ...
            const name = typeof model.display_name === "string"
                ...
                : typeof scope.surface === "string"
                    ? scope.surface
                    : "Model";
            const scoped = { label: `${name} weekly`, usedPercent, resetsAt, ...graded };
            if (typeof model.display_name === "string")
                scoped.model = model.display_name;
            others.push(scoped);
        }
    ...
    if (session)
        ...
        legacy(row.five_hour, "5-hour", "session");
    if (weekly)
        out.push(weekly);
    else
        legacy(row.seven_day, "Weekly", "weekly");
    out.push(...others);
    return out;
}
/**
