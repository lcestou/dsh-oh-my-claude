 ...
 */
export const typedPrompt = (text) => text
    .replace(/\n*Current runtime context\.[\s\S]*$/, "")
    .replace(/^(?:The approval policy changed from "[^"]*" to "[^"]*" \(changed by the user\)\.\s*)+/, "")
    .trim();
...
export const lastModelOf = (folded) => folded.turns.flatMap((t) => t.steps).findLast((s) => s.model !== undefined)?.model;
 ...
 */
export function settingsEvents(pick, time, seq) {
    const out = [];
    const push = (type, data) => out.push({ type, seq: seq + out.length, time, data });
