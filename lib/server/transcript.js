...
export function truncateBytes(text, max) {
    ...
    for (const ch of text) {
        if (Buffer.byteLength(out + ch) > max)
            break;
        out += ch;
    }
    return out;
...
export async function listTranscripts(dir, exclude = new Set()) {
    ...
    for (const name of names) {
        ...
        for (const line of head.lines) {
            ...
            if (!rec)
                continue;
            if (cwd === undefined && typeof rec.cwd === "string")
                cwd = rec.cwd;
            ...
            if (!text)
                continue;
            if (turns === 0 && isAuxPrompt(text))
                break;
            turns += 1;
            if (turns === 1)
                createdAt = timeOf(rec, createdAt);
            if (!title && !isNoise(text))
        ...
        if (rec.type === "user") {
            ...
            if (results.length) {
                ...
                for (const r of results)
                    step.results.set(typeof r.tool_use_id === "string" ? r.tool_use_id : String(r.tool_use_id), {
        ...
        t.steps.forEach((s, j) => {
            ...
            for (const c of s.calls) {
                ...
                const r = s.results.get(c.id) ?? {
                    content: [{ type: "text", text: "No result recorded: the session ended here." }],
                    isError: true,
                    time: s.time,
                };
                const block = {
