...
export function profileFromPath(path, fallback = "web") {
...
export function updateCommand(name, profile) {
...
export function dshFloor(packageJsonText) {
...
const prereleaseIds = (v) => v.trim().split("-").slice(1).join("-").split(".");
...
export function atLeast(version, floor) {
    ...
    if (aPat !== bPat)
        return aPat > bPat;
    if (!aPre)
        return true; // a release reaches any prerelease of its number
    if (!bPre)
        return false; // a prerelease never reaches its release
    ...
    for (let i = 0; i < Math.max(at.length, bt.length); i++) {
        ...
        const y = bt[i];
        if (x === undefined)
            return false; // fewer identifiers ranks lower
        if (y === undefined)
            return true;
        if (x === y)
            continue;
        ...
        if (xn !== yn)
            return yn; // numbers rank below words
        return x > y;
    }
    return true;
}
 ...
 */
export async function latestRelease(name, fetchFn = fetch, now = Date.now(), timeoutMs = 2500) {
    ...
    try {
        ...
        if (r.ok) {
            ...
            if (v && parse(v))
                value = { version: v, dshFloor: dshFloor(text) };
        }
    }
    catch {
...
export async function latestVersion(name, fetchFn = fetch, now = Date.now(), timeoutMs = 2500) {
