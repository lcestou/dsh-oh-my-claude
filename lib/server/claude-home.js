...
import { copyFileSync, existsSync, lstatSync, mkdirSync, readdirSync, renameSync, rmSync, symlinkSync, } from "node:fs";
 ...
 */
export function mirrorPlan(real, mirror) {
    ...
    for (const name of real) {
        ...
        if (at === undefined)
            link.push(name);
        else if (at.forked === true)
            repair.push(name);
    }
    return { link, repair };
}
/** How the mirror dir looks right now, in the shape `mirrorPlan` reads. */
const mirrorState = (dir) => readdirSync(dir).map((name) => {
 ...
 * millisecond.
 */
export function buildMirror(realHome, mirror, log) {
