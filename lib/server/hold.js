...
import { CHILD_ENV, nodeSpawner, shq, sshArgs, } from "./process.js";
 ...
 */
export function holdStartScript(name, cwd, command, args, token) {
    ...
    return (`d=${dirOf(name)}; mkdir -p "$d" && mkfifo "$d"/in || exit 70; ` +
 ...
 */
export function holdAttachScript(name, offset) {
    return (`d=${dirOf(name)}; [ -f "$d"/pid ] || exit ${GONE}; p=$(cat "$d"/pid); ` +
...
export const holdAliveScript = (name) => `d=${dirOf(name)}; [ -f "$d"/pid ] && kill -0 "$(cat "$d"/pid)" 2>/dev/null`;
/** SIGTERM the whole session (the pgid is the leader's pid), SIGKILL what is left five seconds on. */
export const holdKillScript = (name) => `d=${dirOf(name)}; p=$(cat "$d"/pid 2>/dev/null) || exit 0; ` +
...
export const holdCleanScript = (name) => `rm -rf ${dirOf(name)}`;
export function parseHoldLine(line, ready) {
    if (!ready)
        return line === READY ? { kind: "ready" } : undefined;
    if (line.startsWith(EXIT)) {
        const code = Number.parseInt(line.slice(EXIT.length), 10);
        return { kind: "exit", code: Number.isNaN(code) ? 255 : code };
    }
    if (line.startsWith(ERR))
        return { kind: "err", line: line.slice(ERR.length) };
    return { kind: "out", line };
}
/** How long to wait before reattaching after the ssh dropped without an exit line, by try. */
export const reattachDelay = (attempt) => Math.min(2000 * 2 ** attempt, 15_000);
export const sshRunner = (host) => (script) => nodeSpawner("ssh", sshArgs(host, script), ".");
/**
 ...
 */
export function holdHandle(run, name, offset, onOffset, onExit) {
    ...
    const attach = () => {
        ...
        const onLine = (raw) => {
            if (ended)
                return; // the streams are closed; a late line has nowhere to go
...
export function asHoldRecord(raw) {
    ...
    return {
        ...
        name: r.name,
        command: r.command,
        args: r.args.filter((a) => typeof a === "string"),
        cwd: r.cwd,
