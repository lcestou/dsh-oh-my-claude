...
import { pluginErrorsOf } from "./plugins.js";
import { serverIsChinese, serverText } from "./locale.js";
// ---------------------------------------------------------------------------
...
const langOf = (path) => {
    ...
    return ext in EXT_LANG ? (EXT_LANG[ext] ?? "") : "";
};
...
const shellId = (inp) => asStr(inp.bash_id ?? inp.shell_id);
...
export function capLines(body, max = MAX_BODY_LINES) {
...
const words = (name) => cap(name.replaceAll("_", " "));
...
export function formatToolCall(name, inputJson) {
    ...
    switch (name) {
        ...
        default: {
            ...
            const pretty = Object.keys(inp).length > 0
                ? JSON.stringify(inp, (_k, v) => (typeof v === "string" ? clip(v, 300) : v), 2)
                : inputJson;
...
export function formatToolResult(name, filePath, body, isError) {
...
const countOf = (v) => (typeof v === "number" && v > 0 ? v : 0);
...
const nextHeartbeat = (elapsed) => HEARTBEAT_STEPS.find((s) => s > elapsed) ??
...
const nextThinkStep = (tokens) => THINK_STEPS.find((s) => s > tokens) ?? (Math.floor(tokens / THINK_REPEAT) + 1) * THINK_REPEAT;
...
export function resetClock(ms, zone = Intl.DateTimeFormat().resolvedOptions().timeZone) {
    ...
    const clock = { hour: "numeric", minute: "2-digit", timeZone: zone };
    const opts = away <= 24 * 60 * 60 * 1000
        ? clock
    ...
    const text = new Date(ms)
        ...
        .replace(":00", "")
        .replace(/\s([AP]M)/i, (_, half) => half.toLowerCase());
    return `${text} (${zone})`;
}
...
export class Translator {
    ...
    finished;
    denied;
    autoDenied; // tool calls Claude Code refused because a non-interactive run cannot ask
    toolPending; // a tool_use block closed and its result has not arrived yet
    calls = new Set(); // tool_use ids of the streamed message not yet echoed as results
    tokened = false; // the step's first-token marker has gone out
    nested = false; // the frame in hand belongs to a nested agent: its modes are not the line's
    ...
    constructor({ toolActivity = true, continueAfterLimit = false, timeZone, toolTextLimit = TOOL_TEXT_LIMIT, relay = false, dshIds, relayed, log, onToolCall, onToolResult, onResult, redact, onInit, onProgress, onModel, hostLabel, statusNote, } = {}) {
        this.log = log ?? (() => { });
    ...
    translate(event) {
        ...
        switch (event?.type) {
            case "system": {
                ...
                if (event.subtype === "status") {
                    ...
                    if (event.status === "requesting") {
                        if (!this.nested)
                            this.onProgress?.({ mode: "requesting" });
                        return [];
                    }
                    ...
                    if (event.compact_result === "failed") {
                        ...
                        return this.wholeBlock("reasoning", serverText("compactFailed", {
                ...
                if (event.subtype === "task_progress") {
                    ...
                    if (!summaryChanged && !toolNameChanged)
                        return [];
                ...
                if (event.subtype === "model_fallback") {
                    ...
                    return this.wholeBlock("reasoning", said ? `⚠ ${clip(said)}` : `⚠ ${serverText("modelFallback")}${pair}${why}`);
                }
                ...
                if (event.subtype === "model_refusal_fallback") {
                    ...
                    return this.wholeBlock("reasoning", said ? `⚠ ${clip(said)}` : `⚠ ${serverText("modelSwitched")}${pair}${why}`);
                }
                ...
                if (event.subtype === "model_refusal_no_fallback") {
                    ...
                    const why = cat ? ` (${cat} safeguard)` : "";
                    return this.wholeBlock("reasoning", said
                        ? `⛔ ${clip(said)}`
                ...
                if (event.subtype === "model_consent_fallback") {
                    ...
                    const pair = from && to ? `: ${from} → ${to}` : from || to ? `: ${from || to}` : "";
                    return this.wholeBlock("reasoning", said ? `⚠ ${clip(said)}` : `⚠ ${serverText("modelSwitched")}${pair}`);
                }
                ...
                if (event.subtype === "informational") {
                    ...
                    if (!said || (!loud && !event.prevent_continuation))
                        return [];
                ...
                if (event.subtype === "permission_denied") {
                    ...
                    const reason = typeof event.decision_reason === "string"
                        ? event.decision_reason
                        : (event.message ?? "");
                    const why = reason.trim();
                    return this.wholeBlock("reasoning", `⚠ ${serverText("denied", { tool })}${why ? `: ${clip(why)}` : ""}`);
                }
                ...
                if (event.subtype === "api_retry" || event.subtype === "api_error") {
                    ...
                    const down = err.is_network_down
                        ...
                        : err.connection
                            ? ` · ${err.connection}`
                            : "";
                    const head = err.formatted ?? err.message ?? `${serverText("apiError")} ${err.status ?? ""}`.trim();
                ...
                return this.wholeBlock("reasoning", `✓ ${serverText("compacted", { parts: parts.join(serverText("listSep")) })}`);
            }
            case "tool_progress":
            ...
            case "stream_event":
                ...
                return this.partial(event.event ?? { type: "" }, (event.parent_tool_use_id ?? undefined) !== undefined);
            case "assistant":
                return this.assistant(event.message?.content ?? [], event.parent_tool_use_id, event.message?.id);
            case "user":
            ...
            case "result": {
                ...
                for (const [, entry] of this.heartbeatBlocks)
                    events.push(...this.endBlock(entry.block));
                ...
                if (Number.isFinite(totalCost) || Number.isFinite(durationMs)) {
                    ...
                    const denied = event.permission_denials;
            ...
            case "rate_limit_event": {
                ...
                const clock = resetMs > 0
                    ? ` · ${serverText("resets", { when: resetClock(resetAt, this.timeZone) })}`
                    : "";
                const waiting = resetMs > 0 && this.continueAfterLimit ? ` · ${serverText("continuingAtReset")}` : "";
                const failure = {
    ...
    takeStepUsage() {
        ...
        const fallback = summed === undefined && this.resultUsage !== undefined
    ...
    partial(ev, subagent = false) {
        ...
        switch (ev.type) {
            case "message_start": {
                ...
                this.open.clear();
                // A nested agent's message (the CLI's Task tool) is not the line's: the parent's Task is
                // still in flight, so the flag holds, and its calls never enter the set, so clearing here
                // would drop the parent's still-open calls (a Task beside a Bash).
                if (!this.nested) {
                    this.setToolPending(false);
                    this.calls.clear();
                }
                // One whitespace token at the step's first message_start, the response's first byte. dsh
                // times a step's first token off the first non-empty delta in its stream, and this plugin
                // streamed none for a silent think (Fable) until the counter's first mark, a thousand
                // tokens in, and none at all for a message that went straight to a tool call, whose block
                // lands whole at its stop: a 23 s step read as 23 s to first token, 0.3 s of decode and
                // 3,980 tokens a second (2026-09-23). dsh draws no block whose text trims to nothing.
                let marker = [];
                if (!this.tokened && !this.nested) {
                    this.tokened = true;
                    const opened = this.startBlock("reasoning", " ");
                    marker = [...opened.events, ...this.endBlock(opened.block)];
                }
                this.closeThinking();
                return [...this.endThinking(), ...marker];
            }
            ...
            default:
                ...
                if (ev?.type === "message_stop" && !this.nested)
                    this.onProgress?.({ mode: "tool-use" });
    ...
    openBlock(apiIndex, cb) {
        ...
        if (cb.type === "text") {
            ...
            if (!this.nested)
                this.onProgress?.({ mode: "responding" });
        }
            ...
            this.onProgress?.(this.nested ? { thinkingOpen: true } : { thinkingOpen: true, mode: "thinking" });
        }
            ...
            if (cb.id && !this.nested)
                this.calls.add(cb.id);
    ...
    assistant(content, parentToolUseId, id) {
        ...
        if (this.sawPartial && (id === undefined || id === this.streamedId))
            return [];
        const events = [];
    ...
    toolProgress(event) {
        ...
        if (retry) {
            ...
            return this.wholeBlock("reasoning", `↻ ${serverText("attemptFailed", { who, count })}${why ? `: ${why}` : ""}${wait}`);
        }
        const id = event.tool_use_id;
        if (!event.heartbeat || !id)
            return [];
        const elapsed = event.elapsed_time_seconds ?? 0;
        const entry = this.heartbeatBlocks.get(id);
        if (!entry) {
            const tag = event.parent_tool_use_id ? "↳ " : "";
            const { block, events } = this.startBlock("reasoning", `${tag}⏱ ${name} running · ${elapsedText(elapsed)}`);
    ...
    thinkingTokens(total) {
        if (this.thinkingBlock?.started)
            return this.endThinking();
        const entry = this.thinking;
        if (!entry) {
            if (total < THINK_FLOOR)
                return [];
    ...
    setToolPending(v) {
        if (this.toolPending !== v) {
            if (v && !this.nested)
                this.onProgress?.({ tool: v, mode: "tool-use" });
            else
                this.onProgress?.({ tool: v });
        }
        this.toolPending = v;
    }
    ...
    closeThinking() {
        if (this.thinkingBlock)
            this.onProgress?.({ thinkingOpen: false });
    ...
    toolResults(content, parentToolUseId) {
        ...
        // from a relay still says true. A nested agent's echo answers its own call, not the line's:
        // the parent's Task is still running, so neither the flag nor the stall clock moves.
        if (this.calls.size === 0 && !this.nested) {
        ...
        for (const b of content) {
            ...
            if (auto)
                this.autoDenied.push(auto[1]);
            else if (b.is_error && DENIED_RE.test(raw))
                this.denied++;
            ...
            if (!this.onToolResult && this.callNames.has(toolUseId)) {
                ...
                try {
                    ...
                    filePath = asStr(JSON.parse(argsJson).file_path);
                }
                catch {
                    filePath = "";
                }
                events.push(...this.wholeBlock("text", formatToolResult(name, filePath, body, b.is_error ?? false)));
                continue;
            }
