...
export class Translator {
        ...
        switch (event?.type) {
            case "system": {
                ...
                if (event.subtype === "model_fallback") {
                    ...
                    return this.wholeBlock("reasoning", said ? `⚠ ${clip(said)}` : `⚠ Model fallback${pair}${why}`);
                }
                ...
                if (event.subtype === "model_refusal_fallback") {
                    ...
                    return this.wholeBlock("reasoning", said ? `⚠ ${clip(said)}` : `⚠ Model switched${pair}${why}`);
                }
                ...
                if (event.subtype === "model_refusal_no_fallback") {
                    ...
                    const why = cat ? ` (${cat} safeguard)` : "";
                    return this.wholeBlock("reasoning", said ? `⛔ ${clip(said)}` : `⛔ Request blocked${from ? ` on ${from}` : ""}${why}`);
                }
                ...
                if (event.subtype === "api_retry" || event.subtype === "api_error") {
                    ...
                    // connection itself is down, which is worth naming, since that reads as the model hanging.
                ...
                if (Number.isFinite(meta.pre_tokens)) {
                    ...
                    if (Number.isFinite(meta.post_tokens))
                        parts.push(`${meta.post_tokens} after`);
                }
                if (Number.isFinite(meta.duration_ms)) {
                    parts.push(`${(Number(meta.duration_ms) / 1000).toFixed(1)}s`);
                }
                return this.wholeBlock("reasoning", `✓ Context compacted by Claude Code (${parts.join(", ")})`);
            }
            case "tool_progress":
            ...
            case "rate_limit_event": {
                ...
                const waiting = resetMs > 0 && this.continueAfterLimit
    ...
    toolProgress(event) {
        ...
        if (retry) {
            ...
            return this.wholeBlock("reasoning", `↻ ${who} attempt${count} failed${why ? `: ${why}` : ""}${wait}`);
        }
