...
const label = (name) => `${TOOL_ICON.get(name) ?? "◆"}${HEADER_MARK} ${name.startsWith("web_") ? `Web ${name.slice(4)}` : cap(name)}`;
...
export class Translator {
    ...
    constructor({ toolActivity = true, continueAfterLimit = false, timeZone, toolTextLimit = TOOL_TEXT_LIMIT, relay = false, dshIds, relayed, log, onToolCall, onToolResult, onResult, redact, onInit, hostLabel, } = {}) {
        this.log = log ?? (() => { });
