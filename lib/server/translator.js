...
export class Translator {
    ...
    constructor({ toolActivity = true, continueAfterLimit = false, timeZone, toolTextLimit = TOOL_TEXT_LIMIT, relay = false, dshIds, relayed, log, onToolCall, onToolResult, onResult, redact, onInit, onProgress, hostLabel, statusNote, } = {}) {
        this.log = log ?? (() => { });
                ...
                if (event.subtype === "init") {
                    ...
                    if (Array.isArray(event.tools))
                        for (const t of event.tools)
                            if (String(t) === t)
                                tools.push(t);
                    if (names.length > 0 || tools.length > 0)
                        this.onInit?.(names, tools);
                    return [];
                }
