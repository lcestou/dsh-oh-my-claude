...
const makeSnippet = (text, firstWord, radius) => {
...
export function searchTranscript(text, words, snippetRadius) {
    ...
    for (const line of text.split("\n")) {
        ...
        if (!record)
            continue;
        if (!words.every((word) => record.text.toLowerCase().includes(word)))
            continue;
