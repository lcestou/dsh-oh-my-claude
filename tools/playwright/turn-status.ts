...
if (await cand.count()) await cand.click({ force: true });
  ...
  await p
    ...
    .catch((e: Error) => console.log("click failed:", e.message.split("\n")[0]));
...
console.log(
  ...
  n,
  n
    ? `text=${JSON.stringify(await st.innerText())} attr=${await st.getAttribute("data-dsh-oh-my-claude-turn")}`
    : "",
);
if (n) {
  await st.scrollIntoViewIfNeeded();
  await st.screenshot({ path: out.replace(".png", "-crop.png") });
}
