...
const shot = async (p: Page, name: string, clip: Box) => { await p.screenshot({ path: `${out}/${name}.png`, clip }); console.log("wrote", `${name}.png`); };
...
const hit = async (loc: Locator) => { await loc.click({ force: true }); };
...
if (!process.env.PW_CLIPS_ONLY) {
  ...
  if (await addServer.count()) {
    ...
    const add = await boxOf(p.locator(`${panel()} button`, { hasText: /^Add$/ }).first(), "the MCP Add button");
...
if (!process.env.PW_CLIPS_ONLY) {
  ...
  const p = await ctx.newPage();
  await p.goto(dshUrl(token), { waitUntil: "networkidle" }); await wait(p, 2500);
