...
const widths: [string, Size, boolean][] = [
...
for (const [name, vp, mobile] of widths) {
  ...
  console.log(
    ...
    "button:",
    n,
    bb && JSON.stringify({ x: Math.round(bb.x), y: Math.round(bb.y), w: Math.round(bb.width) }),
  );
  if (n) {
    await btn.click();
    ...
    console.log(
      ...
      dBox &&
        JSON.stringify({
          x: Math.round(dBox.x),
          y: Math.round(dBox.y),
          w: Math.round(dBox.width),
          h: Math.round(dBox.height),
        }),
    );
  ...
  await p.screenshot({
    path: `${process.env.PW_OUT ?? "/tmp"}/composer-${name}.png`,
    fullPage: false,
  });
  await ctx.close();
}
