...
const rows = (p, text) => p.locator('[role="treeitem"]').filter({ hasText: text });
...
const composerClip = async (p, h = 420, left = 60) => {
  const button = await p.locator('button[aria-label="Oh My Claude"]').boundingBox();
  const dialog = await p.locator(panel()).boundingBox().catch(() => null);
  const pad = 14;
...
const hit = async (loc) => { await loc.click({ force: true }); };

// Desktop stills
if (!process.env.PW_CLIPS_ONLY) {
  const ctx = await b.newContext({ viewport: { width: 1400, height: 900 }, deviceScaleFactor: 2 });
  const p = await open(ctx);
  await hit(p.locator('button[aria-label="Oh My Claude"]')); await wait(p, 1500);
  for (const t of ["Memory", "Rewind", "Changes", "Asides"]) { await hit(tab(p, t)); await wait(p, 1500); await shot(p, `panel-${t.toLowerCase()}`, await composerClip(p)); }
  // The MCP list is whatever servers this box has, so the shot is of the Add form instead.
  await hit(tab(p, "MCP")); await wait(p, 1500);
  const addServer = p.locator(`${panel()} button`, { hasText: /^Add server/ }).first();
  if (await addServer.count()) {
    await hit(addServer); await wait(p, 1200);
    ...
    await p.locator(`${panel()} textarea[placeholder^="args"]`).fill("@example/recipes-mcp\n--db\n./recipes.sqlite");
    await wait(p, 600);
    ...
    const top = await p.locator(`${panel()} select`).first().boundingBox();
    const add = await p.locator(`${panel()} button`, { hasText: /^Add$/ }).first().boundingBox();
    const box = await p.locator(panel()).boundingBox();
    let right = top.x + top.width;
    for (const f of await p.locator(`${panel()} select, ${panel()} input, ${panel()} textarea`).all()) {
    ...
    await shot(p, "panel-mcp", { x: top.x - 12, y: top.y - 16, width: right - top.x + 12, height: add.y + add.height - top.y + 32 });
  }
  ...
  await hit(shield); await wait(p, 1200);
  // Frame the menu itself, not a fixed window above the composer: anything higher is the conversation.
  {
    const m = await p.locator('[role="menu"]').last().boundingBox();
    const bt = await p.locator('button[aria-label="Oh My Claude"]').boundingBox();
    ...
    const y = m.y + 8;
    await shot(p, "shield-menu", { x: Math.max(0, x0 - 14), y, width: Math.min(x1 - x0 + 28, 1400 - Math.max(0, x0 - 14)), height: bt.y + bt.height - y + 16 });
  }
  await p.keyboard.press("Escape"); await wait(p, 300);
  const stats = p.locator("div").filter({ hasText: /\d+ turns\s*·?\s*\d+ steps/ }).last();
  ...
  if (await ring.count()) { await hit(ring); await wait(p, 2500); const d = p.locator('[role="dialog"]').last(); const db = await d.boundingBox(); if (db) await shot(p, "context-usage", { x: db.x - 10, y: db.y + 46, width: db.width + 20, height: db.height - 46 }); await p.keyboard.press("Escape"); }
  await ctx.close();
}
// Phone stills
if (!process.env.PW_CLIPS_ONLY) {
  ...
  await hit(p.locator('button[aria-label="Oh My Claude"]')); await wait(p, 1500); await hit(tab(p, "Memory")); await wait(p, 1200);
