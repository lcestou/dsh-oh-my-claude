...
A text steer you typed also shows in a card above the composer until Claude reads it, with Edit and Remove. Both ask the CLI for the message back first (`cancel_async_message`). When the CLI answers that it already took it, nothing changes and the card says so. Otherwise the edited text goes to the CLI and dsh's pending message is replaced to match, so Claude and the chat read the same words. dsh's own Queue mode already lets you edit a queued message; this card covers Steer, where the plugin hands the message to Claude straight away.
