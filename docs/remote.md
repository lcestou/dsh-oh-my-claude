...
Removing a box removes the workspaces pinned to it, and its row says how many before the click. Their sessions keep their logs, as dsh's delete leaves them: dsh lists them under Ungrouped, and a message sent in one is refused with a note saying its workspace was removed, since there is no box left to run it on. Pin the same folder on the same box again and they come back under the workspace and run there. A workspace dsh refuses to delete stays in the card with the error, since the sidebar still lists it.
