...
- Every function, method, class and React component has a `/** … */` comment directly above it that says what its name does not: the case it gets wrong, what it assumes, what it returns or throws on failure, or why it exists. A comment that restates the name is noise and does not count. This is a standard the codebase is being brought up to, not one the gate checks yet: about 540 functions still lack one (counted 2026-09-21), and a lint rule to enforce it follows the backfill.
...
- The `jsx-a11y` rules run in the gate: a button with no name, an image with no `alt`, an invalid link, an unknown ARIA attribute and the rest fail the build. Four are off for now, each because the fix it asks for changes how something looks and needs a person to check it: `prefer-tag-over-role` (a `<dialog>` is hidden unless it has `open`, and an `<output>` is inline where our status lines are blocks), `no-autofocus` (focusing an editor the person just opened is usually right), and the two element-interaction rules. They come back on with the change that fixes their twenty sites.
