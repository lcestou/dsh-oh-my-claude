...
// --- terminal turns: the transcript check resolves the local path, reads past the baseline, and
// stays out of the way on a remote box ---
{
  const { mkdtemp, writeFile: write, stat: st } = await import("node:fs/promises");
  const { tmpdir } = await import("node:os");
  const { join: j } = await import("node:path");
  const home = await mkdtemp(j(tmpdir(), "omc-home-"));
  const cwd = j(home, "ws");
  const a = new ClaudeCodeAdapter(fakeCtx({ on() {} }), Config({}));
  a.claudeHome = home;
  const prep = { cwd, session: { id: "c1", resuming: true } } as unknown as TurnPrep;
  const path = a.transcriptPath(prep)!;
  assert.equal(path, j(home, "projects", projectDirName(cwd), "c1.jsonl"));
  assert.equal(a.transcriptPath({ cwd } as unknown as TurnPrep), undefined, "no session id");
  const remote = new ClaudeCodeAdapter(fakeCtx({ on() {} }), Config({ sshHost: "lilly" }));
  assert.equal(remote.transcriptPath(prep), undefined, "a remote transcript is not watched");
  const proc = fakeProc({ alive: true });
  const row = (o: object) => JSON.stringify(o);
  const T = "2026-09-10T22:00:00.000Z";
  await (await import("node:fs/promises")).mkdir(j(path, ".."), { recursive: true });
  await write(
    path,
    `${row({ type: "user", uuid: "d1", timestamp: T, entrypoint: "dsh-oh-my-claude", message: { role: "user", content: "ours" } })}\n`,
  );
  assert.deepEqual(await a.terminalTurnsSince(proc, prep), [], "no baseline yet: nothing reported");
  await a.markTranscriptSeen(proc, prep);
  assert.equal(proc.transcriptSeen, (await st(path)).size, "baseline is the file size");
  assert.deepEqual(await a.terminalTurnsSince(proc, prep), [], "nothing new since the baseline");
  const terminal = [
    row({
      type: "user",
      uuid: "t1",
      timestamp: T,
      entrypoint: "cli",
      message: { role: "user", content: "typed in a terminal" },
    }),
    row({
      type: "assistant",
      uuid: "t2",
      ...
      timestamp: T,
      entrypoint: "cli",
      message: { id: "m2", role: "assistant", content: [{ type: "text", text: "answered there" }] },
    }),
  ].join("\n");
  await write(
    path,
    `${await (await import("node:fs/promises")).readFile(path, "utf8")}${terminal}\n`,
  );
  const turns = await a.terminalTurnsSince(proc, prep);
  assert.equal(turns.length, 1, "the terminal exchange past the baseline is reported");
  assert.equal(turns[0]!.content[0]!.text, "typed in a terminal");
