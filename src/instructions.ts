...
const resolveImport = (path: string, from: string): string =>
  path.startsWith("~/") ? join(homedir(), path.slice(2)) : resolve(dirname(from), path);

const statFile = async (path: string): Promise<{ size: number; mtime: number } | null> => {
  try {
...
const rulesIn = async (dir: string): Promise<string[]> => {
...
export async function listInstructions(
  ...
  const add = async (path: string, kind: InstructionKind, depth = 0, importedBy?: string) => {
    ...
    const file: InstructionFile = { path: at, kind, size: found.size, mtime: found.mtime };
  ...
  for (const rule of await rulesIn(join(claudeHome, "rules"))) await add(rule, "User");
  ...
  for (const dir of chain) {
    ...
    for (const rule of await rulesIn(join(dir, ".claude", "rules"))) await add(rule, "Project");
