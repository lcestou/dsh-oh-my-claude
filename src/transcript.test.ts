...
{
  const { foreignTurns, foreignTurnsBlock, readTranscriptFrom } = await import("./transcript.js");
  ...
  const ours = [
    row({
      ...
      timestamp: T,
      entrypoint: own,
      message: { role: "user", content: "from dsh" },
    }),
    row({
      type: "assistant",
      uuid: "d2",
      ...
      timestamp: T,
      entrypoint: own,
      message: { id: "m1", role: "assistant", content: [{ type: "text", text: "dsh reply" }] },
    }),
  ].join("\n");
  const terminal = [
    ...
    row({
      ...
      timestamp: T,
      entrypoint: "cli",
      message: { id: "m2", role: "assistant", content: [{ type: "text", text: "terminal reply" }] },
    }),
    row({
      type: "user",
      uuid: "t3",
  ...
  // Only the terminal's completed exchange counts: ours are skipped, bookkeeping rows have no
  // stamp, and the unanswered trailing prompt is a turn still running.
  const turns = foreignTurns(`${ours}\n${terminal}\n`, own);
  assert.equal(turns.length, 1, "one completed terminal turn");
  assert.equal(turns[0]!.content[0]!.text, "hello from terminal\nline two");
  assert.deepEqual(foreignTurns(`${ours}\n`, own), [], "nothing foreign reads as no turns");
  assert.deepEqual(foreignTurns("", own), [], "an empty tail reads as no turns");
  const block = foreignTurnsBlock(turns, 1000);
  assert.match(block, /^⇄ 1 exchange in a terminal/, "header counts exchanges");
  assert.match(block, /\n> hello from terminal\n> line two\n/, "every prompt line is quoted");
  assert.match(block, /\nterminal reply$/, "the reply follows the prompt");
  assert.match(foreignTurnsBlock(turns, 5), /> hello\n/, "each side is cut at the limit");
  ...
  await writeFile(path, `${ours}\n${terminal}\n`);
  assert.equal(await readTranscriptFrom(path, seen), `${terminal}\n`, "only the new bytes");
