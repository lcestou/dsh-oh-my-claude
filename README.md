...
**Errors.** Abort from the UI kills the child. Non-zero exits surface with the last stderr; a real usage limit surfaces as `RATE_LIMIT` with the provider's reset time as retry-after, worded like the CLI ("You've hit your usage limit · continuing automatically at 7pm (America/New_York)"); the clock is in the browser's zone dsh stamps on each prompt, or the box's zone when no prompt carried one. While the CLI retries a 429 or 5xx by itself, each attempt is one reasoning line with the wait and the attempt count, so the turn never looks busy for nothing. With `continueAfterLimit` on, the plugin arms a timer for the reset (kept in `limit-waits.json`, so a restart re-arms it) and then drops a continue notice through the same path a restart uses; any prompt sent before then cancels the wait. Before the notice goes out the plugin checks that the session still runs on this provider (a reroute to another model drops it) and asks the usage endpoint whether a window is still at its cap (then the wait is re-armed for that reset).
