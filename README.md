...
**Command bridge.** The CLI's init frame lists its slash commands (skills, custom commands, built-ins such as `/compact`). Each name dsh's command grammar accepts is registered as a dsh `/command` on first sight (default instance only; a name dsh or another plugin already owns is skipped), so the composer's slash menu offers `/verify`, `/summarize-changes` and the rest. Running one hands `/name <arguments>` to Claude as the next prompt, where the CLI expands it exactly as the terminal would; the composer shows `/name sent to Claude Code`. Switch off with `commandBridge: false`.
