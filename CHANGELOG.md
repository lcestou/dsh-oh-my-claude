...
- dsh's Session statistics for a Claude session: time to first token now runs to the moment
  Claude starts thinking, and tokens per second counts the think, where before a silent think or
...
- On dsh 0.1.7 (session format v4) the plugin's own wake, restart, reconnect and usage-limit
  ...
  The notice now carries the source kind v4 expects; v3 logs keep the old shape.
